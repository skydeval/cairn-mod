...
- subscribeLabels retention sweep ([§F4](cairn-design.md#f4-comatprotolabelsubscribelabels-endpoint)): daily at the configured UTC hour the writer task runs batched DELETEs against `labels` rows older than `[subscribe].retention_days` (default 180); operator-initiated runs via `tools.cairn.admin.retentionSweep` admin XRPC (admin-role-only, audited per call) and the `cairn retention sweep` CLI; new `[retention]` config block carries sweep execution policy (`sweep_enabled`, `sweep_run_at_utc_hour`, `sweep_batch_size`); the read-side `OutdatedCursor` floor is computed by live SQL filter and is invariant under sweep — `tests/writer.rs::sweep_does_not_change_subscriber_visible_floor` guards the contract (#12)
- E2E quickstart rot-check: new [`tests/e2e/quickstart.sh`](tests/e2e/quickstart.sh) walks the README's operator workflow end-to-end (signing-key generation → config → `publish-service-record` → `serve` → `curl /.well-known/did.json`) against a [mock PDS binary](examples/mock_pds.rs) so silent README drift fails CI. New `e2e-quickstart` CI job. The one mock shortcut: `cairn operator-login` is bypassed (rpassword reads from /dev/tty which CI lacks), with the operator session file written directly using the canned tokens the mock mints — flagged in the script's "Mocking shortcut" comment block (#10)
- contrib syntax smoke check: new `contrib-smoke` CI job runs `systemd-analyze verify` on `cairn.service`, `caddy validate` on `Caddyfile`, and `nginx -t` on `cairn.conf` (wrapped in a minimal events+http shell since the snippet declares http-block directives without the top-level wrappers). End-to-end deployment scenarios with real TLS / header forwarding / IP propagation track separately as chainlink #32 (#9)
