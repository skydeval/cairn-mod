...
9. **Transitive trust of upstream PDSes for report origination.** v1.7's `com.atproto.moderation.createReport` inbound flow (via `xrpc_gateway`, §F23 / §A10) accepts reports forwarded by upstream PDSes that the operator has added to `xrpc_trusted_pdses`. The PDS asserts which user originated the report via the request body's `reportedBy` field; cairn-mod cannot cryptographically verify this assertion, because the JWT is signed by the PDS, not by the user. Operators trust the upstream PDS to assert truthfully. A compromised or malicious upstream PDS could submit reports under arbitrary user DIDs, including DIDs that have never interacted with that PDS. Mitigation: only add upstream PDSes to `xrpc_trusted_pdses` if the operator trusts the PDS's correct operation. Removing a PDS from the table immediately stops accepting forwarded reports from it (the membership check runs per request). This trust gesture is distinct from the user-direct intake at `/xrpc/com.atproto.moderation.createReport` (§F11), which derives `reportedBy` from `claims.iss` (the user authenticates directly) and applies its own per-DID rate limits + suppression. The gateway path intentionally does not replicate those gates — they're substituted by the trust-table membership check.
